import pytest
from unittest.mock import patch, AsyncMock
from src.agent.graph import build_graph, should_continue_execution
from src.agent.state import PlanStep

def test_should_continue_execution():
    # Multi-step, unfinished
    assert should_continue_execution({"current_step": 0, "plan": [PlanStep(id=1, tool_name="a", arguments={}, reason="b")]}) == "executor"
    # Completed
    assert should_continue_execution({"current_step": 1, "plan": [PlanStep(id=1, tool_name="a", arguments={}, reason="b")]}) == "synthesizer"
    # Empty plan
    assert should_continue_execution({"current_step": 0, "plan": []}) == "synthesizer"

@pytest.mark.asyncio
async def test_graph_routing_single_step():
    with patch("src.agent.graph.planner_node", new_callable=AsyncMock) as mock_planner, \
         patch("src.agent.graph.executor_node", new_callable=AsyncMock) as mock_executor, \
         patch("src.agent.graph.synthesizer_node", new_callable=AsyncMock) as mock_synthesizer:
        
        mock_planner.return_value = {"plan": [PlanStep(id=1, tool_name="test", arguments={}, reason="test")]}
        
        def executor_side_effect(state, *args, **kwargs):
            step = state.get("current_step", 0)
            plan = state.get("plan", [])
            if step >= len(plan):
                return {}
            return {"current_step": step + 1}
            
        mock_executor.side_effect = executor_side_effect
        mock_synthesizer.return_value = {"draft_response": "done"}
        
        graph = build_graph()
        initial_state = {"query": "test", "current_step": 0}
        
        result = await graph.ainvoke(initial_state)
        
        assert mock_planner.call_count == 1
        assert mock_executor.call_count == 1 # 1 real step
        assert mock_synthesizer.call_count == 1
        assert result["current_step"] == 1

@pytest.mark.asyncio
async def test_graph_routing_multi_step():
    with patch("src.agent.graph.planner_node", new_callable=AsyncMock) as mock_planner, \
         patch("src.agent.graph.executor_node", new_callable=AsyncMock) as mock_executor, \
         patch("src.agent.graph.synthesizer_node", new_callable=AsyncMock) as mock_synthesizer:
        
        mock_planner.return_value = {"plan": [
            PlanStep(id=1, tool_name="test1", arguments={}, reason="test"),
            PlanStep(id=2, tool_name="test2", arguments={}, reason="test"),
            PlanStep(id=3, tool_name="test3", arguments={}, reason="test")
        ]}
        
        def executor_side_effect(state, *args, **kwargs):
            step = state.get("current_step", 0)
            plan = state.get("plan", [])
            if step >= len(plan):
                return {}
            return {"current_step": step + 1}
            
        mock_executor.side_effect = executor_side_effect
        mock_synthesizer.return_value = {"draft_response": "done"}
        
        graph = build_graph()
        initial_state = {"query": "test", "current_step": 0}
        
        result = await graph.ainvoke(initial_state)
        
        assert mock_planner.call_count == 1
        assert mock_executor.call_count == 3 # 3 real steps
        assert mock_synthesizer.call_count == 1
        assert result["current_step"] == 3

@pytest.mark.asyncio
async def test_graph_routing_empty_plan():
    with patch("src.agent.graph.planner_node", new_callable=AsyncMock) as mock_planner, \
         patch("src.agent.graph.executor_node", new_callable=AsyncMock) as mock_executor, \
         patch("src.agent.graph.synthesizer_node", new_callable=AsyncMock) as mock_synthesizer:
        
        mock_planner.return_value = {"plan": []}
        
        def executor_side_effect(state, *args, **kwargs):
            step = state.get("current_step", 0)
            plan = state.get("plan", [])
            if step >= len(plan):
                return {}
            return {"current_step": step + 1}
            
        mock_executor.side_effect = executor_side_effect
        mock_synthesizer.return_value = {"draft_response": "done"}
        
        graph = build_graph()
        initial_state = {"query": "test", "current_step": 0}
        
        result = await graph.ainvoke(initial_state)
        
        assert mock_planner.call_count == 1
        assert mock_executor.call_count == 1 # 1 terminal evaluation
        assert mock_synthesizer.call_count == 1
        assert result["current_step"] == 0
